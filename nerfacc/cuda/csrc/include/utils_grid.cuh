...
// see: https://zhuanlan.zhihu.com/p/405062794
...
{    
    ...
    if (ray.inv_dir.x >= 0) {
        tmin = (aabb.min.x - ray.origin.x) * ray.inv_dir.x;  // * ray.inv_dir.x is to unify the reference to the ray for x,y,z
...
{     
    ...
    // traverse完一个binary的voxel体素后，到下一个voxel体素走完，在三个维度上有多少距离
...
// binary某个voxel体素已经跑完了，要进入下一个voxel体素
inline __device__ bool single_traversal(
    ...
    // 在可供下降空间最小的轴上更新
    // 该函数首先比较 tdist 中的三个分量，以确定哪个轴方向的下一个交点距离最小。
