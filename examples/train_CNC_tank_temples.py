...
parser.add_argument(
    ...
    default=str(pathlib.Path.cwd() / "data/TanksAndTemple"),
...
for n_features in [args.n_features]:
    for current_scene in [args.scene]:
        if 1:
            from datasets.tanks import SubjectLoader_Tanks
        ...
        train_dataset = SubjectLoader_Tanks(
        ...
        test_dataset = SubjectLoader_Tanks(
            subject_id=current_scene,
            root_fp=args.data_root,
            split="test",
            num_rays=None,
        ...
        aabb = train_dataset.scene_bbox.cuda().view(-1)
        render_step_size = train_dataset.render_step_size
        ...
        for step in range(max_steps + 1):
            ...
            if step > 0 and step % max_steps == 0:
                ...
                if 1:
                    os.makedirs('./results/TanksTemples', exist_ok=True)
                    Q_step = '_binary' if ste_binary else ('_Q' + str(Q))
                    with open('./results/TanksTemples/output.txt', 'a') as fw:
