...
def traverse_grids(
    ...
    rays_mask: Optional[Tensor] = None,  # [n_rays] default: None
...
def _enlarge_aabb(aabb, factor: float) -> Tensor:
    # For NeRF dataset: aabb = [-1.5000, -1.5000, -1.5000, 1.5000, 1.5000, 1.5000], factor = 1
    center = (aabb[:3] + aabb[3:]) / 2  # for NeRF dataset: [0, 0, 0]
    extent = (aabb[3:] - aabb[:3]) / 2  # for NeRF dataset: [1.5, 1.5, 1.5]
